    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        if (hasSubscriptionFlag) {
            try {
                ...
                //如果表达式类型为非tag,构建过滤数据consumerFilterData
        ...
        if (this.brokerController.getBrokerConfig().isFilterSupportRetry()) {
            //支持对重试主题的过滤，不支持对重试主题的属性过滤，也就是如果是tag模式，执行isMatchedByCommitLog方法将直接返回true
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    if (this.brokerController.getBrokerConfig().isTransferMsgByHeap()) {
                        System.out.println("isTransferMsgByHeap()");
                ...
                case ResponseCode.PULL_NOT_FOUND:
                    ...
                    if (brokerAllowSuspend && hasSuspendFlag) {
                        ...
                        this.brokerController.getPullRequestHoldService().suspendPullRequest(topic, queueId, pullRequest);//暂时没有消息，就将请求放入队列挂起
                        response = null;
                        break;
                    }
